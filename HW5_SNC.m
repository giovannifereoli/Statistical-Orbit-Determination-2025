...
% ASSIGNMENT:   Homework 5 - SRIF, w/ Q
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear; clc; close;
...
grid on;
exportgraphics(gca1, 'Orbit_trajectories2.pdf', 'ContentType','image',...
    'Resolution', 1000);
...
grid on;
hold off;
exportgraphics(gca2, 'Radiometric_Residuals2.pdf', 'ContentType','image',...
    'Resolution', 1000);
...
for i = 1:length(sigma_Q_cont_values)
    ...
    % Run the Filter (LKF, or SRIF)
    results = srif(trajectory_ref, sorted_measurements, P0, settings_PN);
...
xlabel('Process Noise Sigma ($km/s^2$)', 'Interpreter', ...
    'latex', 'FontSize', 14);
...
xlabel('Process Noise Sigma ($km/s^2$)', 'Interpreter', ...
    'latex', 'FontSize', 14);
ylabel('RMS Residuals (km, km/s)', 'Interpreter', 'latex', 'FontSize', 14);
legend({'RMS Residual 1', 'RMS Residual 2'}, 'Interpreter', 'latex', ...
    'FontSize', 12, 'Location', 'best');
grid on;
exportgraphics(gca3, sprintf('Optimal_sigma_SNC_%s.pdf', filename_suffix), 'ContentType','image',...
    'Resolution', 1000);

%% Find the optimal sigma_Q_cont, run SRIF
...
optimal_sigma_Q_cont = 0.5 * 1e-8; % sigma_Q_cont_values(optimal_index);
...
% Run the filter 
results_lkf = lkf(trajectory_ref, sorted_measurements, P0, settings_PN);

% Compute and display RMS errors for the optimal sigma
[rms_pos_3d_optimal_lkf, rms_vel_3d_optimal_lkf, rms_residuals_optimal_lkf] = compute_rms_errors(results_lkf, ...
    trajectory_true, sorted_measurements);

fprintf('\n');
disp(['RMS Position (optimal): ', num2str(rms_pos_3d_optimal_lkf)]);
disp(['RMS Velocity (optimal): ', num2str(rms_vel_3d_optimal_lkf)]);
disp(['RMS Residuals (optimal 1): ', num2str(rms_residuals_optimal_lkf(1))]);
disp(['RMS Residuals (optimal 2): ', num2str(rms_residuals_optimal_lkf(2))]);
...
function results = srif(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    for t = 1:T
        ...
        prefit_residual = sorted_measurements(t).residual;
        H_tilde = sorted_measurements(t).partials.wrt_X;
        R_meas = diag(sorted_measurements(t).covariance);
        ...
        if nargin > 3 && ~isempty(settings_PN)
            ...
            [~, gamma_k] = calculate_Q_discrete(dt, x_ref, settings_PN);

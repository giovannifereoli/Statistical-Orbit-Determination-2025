...
% ASSIGNMENT:   Homework 3 - RTS Smoother, Batch Compare
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear; clc; close;
...
% Run the LKF filter, and print RMS
disp("Running LKF...");
results_lkf = lkf(trajectory_ref, sorted_measurements, P0);
[~, ~, ~] = compute_rms_errors(results_lkf, trajectory_true, sorted_measurements);

% Run the smoother, and print RMS
disp("Running RTS...");
results = rts_smoother(results_lkf, trajectory_ref, sorted_measurements, 1);
[~, ~, ~] = compute_rms_errors(results, trajectory_true, sorted_measurements);

% Run Batch filter, and print RMS
disp("Running Batch Filter...");
results_batch = batch_filter(trajectory_ref, sorted_measurements, P0);
[~, ~, ~] = compute_rms_errors(results_batch, trajectory_true, sorted_measurements);

%% Compare Smoother and Batch Filter in Semilogy Scale
...
state_error_smoother = results.state_corrected_hist - true_states_at_meas;
state_error_batch = results_batch.state_corrected_hist - true_states_at_meas;

% Compute 3-sigma bounds from covariance matrix at measurement times
sigma_bounds_smoother = zeros(size(state_error_smoother));
sigma_bounds_batch = zeros(size(state_error_batch));
T_meas = length(measurement_times);
for t = 1:T_meas
    sigma_bounds_smoother(:, t) = 3 * sqrt(diag(results.P_hist(:, :, t)));
    sigma_bounds_batch(:, t) = 3 * sqrt(diag(results_batch.P_hist(:, :, t)));
end

% Compute differences
state_error_diff = abs(state_error_smoother - state_error_batch) ./ abs(state_error_batch);
sigma_diff = abs(sigma_bounds_smoother - sigma_bounds_batch) ./ abs(sigma_bounds_batch);
...
labels_sigma =  {'$\sigma_x$', '$\sigma_y$', '$\sigma_z$', ...
    '$\sigma_{\dot{x}}$', '$\sigma_{\dot{y}}$', '$\sigma_{\dot{z}}$'};
for i = 1:6
    subplot(2,3,i);
    semilogy(measurement_times_hours, state_error_diff(i, :), 'o', ...
        'MarkerSize', 5, 'MarkerFaceColor', colors(i, :), ...
        'MarkerEdgeColor', 'k', 'DisplayName', [labels{i}]);
    hold on;
    semilogy(measurement_times_hours, sigma_diff(i, :), 'diamond', ...
        'MarkerSize', 5, 'MarkerFaceColor', colors(6+i, :), ...
        'MarkerEdgeColor', 'k', 'DisplayName', [labels_sigma{i}]);
    xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
...
% Save the plot
% exportgraphics(gca_comp, sprintf('SmootherVsBatch_%s.pdf', filename_suffix), 'ContentType', 'image', 'Resolution', 1000);
...
function smoothed_results = rts_smoother(lkf_results, trajectory_ref, ...
    sorted_measurements, flag_SNC)
    % Extract dimensions
    ...
    for t = T-1:-1:1
        ...
        % OSS: When no process noise is included, the covariance matrix may 
        % become nearly singular, leading to inversion errors. In such cases,
        % use the analytical solution S = inv(Phi_ti(:,:,ii+1)) for the smoother.
        if flag_SNC == 0

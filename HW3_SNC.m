...
for i = 1:length(sigma_Q_cont_values)
    ...
    settings_PN.frame_type = 'ECI';                    % Frame type (either 'RTI' or 'ECI')
...
% Find the index with the smallest combined difference
[~, optimal_idx] = min(rms_residuals_vals_1 + rms_pos_vals + ...
    rms_residuals_vals_2 + rms_vel_vals);  % Combine the differences
optimal_sigma_Q_cont = sigma_Q_cont_values(optimal_idx);
...
function results = lkf(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    % Precompute process noise matrix if required
    Q_disc_cache = cell(T, 1);  % Cache for Q_disc values
    for t = 1:T
        if nargin > 3
            ...
            Q_disc_cache{t} = calculate_Q_discrete(dt, settings_PN);  % Store Q_disc
        else 
            Q_disc_cache{t} = zeros(6);
        end
    end
...
function Q_disc = calculate_Q_discrete(dt, settings)

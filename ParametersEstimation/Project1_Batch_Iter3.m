...
% ASSIGNMENT:   Project 1 - BATCH - Iter3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear; clc; close;
...
%% Load Corrections
% Load state0_deviation from file
load('state0_deviation_iter2.mat', 'state0_deviation');

% Ensure state0_deviation is a column vector
state0_deviation = state0_deviation(:); % Converts to column vector if needed

% Assign each element to a separate variable
dx    = state0_deviation(1);
dy    = state0_deviation(2);
dz    = state0_deviation(3);
dvx   = state0_deviation(4);
dvy   = state0_deviation(5);
dvz   = state0_deviation(6);
dmu   = state0_deviation(7);
dJ1   = state0_deviation(8);
dJ2   = state0_deviation(9);
dCD   = state0_deviation(10);
dGS1x = state0_deviation(11);
dGS1y = state0_deviation(12);
dGS1z = state0_deviation(13);
dGS2x = state0_deviation(14);
dGS2y = state0_deviation(15);
dGS2z = state0_deviation(16);
dGS3x = state0_deviation(17);
dGS3y = state0_deviation(18);
dGS3z = state0_deviation(19);

%% Create Reference Trajectory

% Constants
coeffs = [0 + dJ1, 1.082626925638815 * 1e-3 + dJ2]; % Zonal harmonic coefficients (i.e., [J1, J2])
R = 6378.1363; % Earth radius 
mu = 398600.4415 + dmu;  % Gravitational parameter 
l_max = 2; % Zonal Harmonics Terms 
A = 3 * 1e-6; % Cross-Sectional Area
m = 970; % Mass
CD = 2.0 + dCD; % Drag Coefficient
param_meas = 9;

% Define the station positions in ECEF at t0
station_ecef0 = containers.Map(...
    {'101', '337', '394'}, ...
    {[-5127.5100 + dGS1x; -3794.1600 + dGS1y; 0.0 + dGS1z], ...
     [3860.9100 + dGS2x; 3238.4900 + dGS2y; 3898.0940 + dGS2z], ...
     [549.5050 + dGS3x; -1380.8720 + dGS3y; 6182.1970 + dGS3z]});

% Initial state (reference orbit)
state0 = [757.7 + dx; 5222.607 + dy; 4851.5 + dz; 2.21321 + dvx; 4.67834 + dvy; -5.3713 + dvz]; % (km, km/s)
...
ylabel('Range Residual $\cdot 1e-5$ (km)', 'Interpreter', 'latex', 'FontSize', 16);
...
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 16);
ylabel('Range-Rate Residual $\cdot 1e-6$ (km/s)', 'Interpreter', 'latex', 'FontSize', 16);
legend('Location', 'northeast', 'FontSize', 12, 'Interpreter', 'latex');
grid on;
hold off;
exportgraphics(gca123, 'Prefit_Batch_Iter3.pdf', 'ContentType',...
...
ylabel('Position Deviation $\cdot 1e-5$ (km)', 'Interpreter', 'latex', 'FontSize', 14);
...
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Velocity Deviation $\cdot 1e-8$ (km/s)', 'Interpreter', 'latex', 'FontSize', 14);
legend('show', 'Interpreter', 'latex', 'FontSize', 14, 'Location', 'northeast');
grid on;
hold off;
subplot(2,2,3);
...
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Deviation $\cdot 1e-4$ (-)', 'Interpreter', 'latex', 'FontSize', 14);
legend('show', 'Interpreter', 'latex', 'FontSize', 14, 'Location', 'northeast');
grid on;
hold off;
subplot(2,2,4);
...
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Deviation $\cdot 1e-6$ (km)', 'Interpreter', 'latex', 'FontSize', 14);
legend('show', 'Interpreter', 'latex', 'FontSize', 14, 'Location', 'northeast');
grid on;
hold off;
...
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Range Residual $\cdot 1e-5$(km)', 'Interpreter', 'latex', 'FontSize', 14);
...
ylabel('Range-Rate Residual $\cdot 1e-6$ (km/s)', 'Interpreter', 'latex', 'FontSize', 14);
legend('show', 'Interpreter', 'latex', 'FontSize', 14);
grid on;
hold off;
...
%% Re-iterate Again and/or Save Solution

% Save correction
state0_deviation = state0_deviation + results.state0_deviation;
save('state0_deviation_iter3.mat', 'state0_deviation');

% Save solution
results_Batch_Iter3 = results;
save('results_Batch_Iter3.mat', 'results_Batch_Iter3');

%% Print out results

% Extract the final covariance matrix
P_initial_batch = results.P_hist(:, :, end);

% Extract diagonal elements (variances)
variances = diag(P_initial_batch);

% Compute standard deviations
std_devs = sqrt(variances);

% Extract the final state values
state_initial_batch = results.state_corrected_hist(:, 1);
...
fprintf('\n%-15s %-20s %-20s\n', 'State', 'Mean Value', 'Initial Std Dev');
fprintf('------------------------------------------------------------\n');
for i = 1:length(state_labels)
    fprintf('%-15s %-25.14f %-25.14f\n', state_labels{i}, state_initial_batch(i), std_devs(i));
end

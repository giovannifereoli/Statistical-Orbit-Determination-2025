...
% todo: buf in reference trajectory 
...
results = lkf(trajectory_ref3, measurements_struct3, P0, settings_PN);
...
function dstate = flyby_ODE(t, state, JD0, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM)
    ...
    % Non‐grav activity law
    g_r = (r_norm / AU)^(-2);
    
    % empirical coefficients (to be estimated)
    A1 = state(8);   % radial
    A2 = state(9);   % transverse
    A3 = state(10);   % normal
    
    % non‐grav acceleration
    a_NG = g_r * (A1 * r_hat + A2 * t_hat + A3 * n_hat);

    % Total acceleration
    a_total = a_earth + a_sun + a_srp + a_NG;

    % Populate derivative vector
    dstate(1:3) = v;
    dstate(4:6) = a_total;
    dstate(7:10) = 0;  % Cr constant
    ...
    if flag_STM == 1
        ...
        %--- now build analytic partials ∂a_NG/∂r and ∂a_NG/∂v ---
        % skew-matrix helper
    
        % 1) ∂g_r/∂r  (row vector)
        Dg = -2*g_r/r_norm * r_hat';    % 1×3
    
        % 2) ∂r̂/∂r
        Drhat = (eye(3) - r_hat*r_hat')/r_norm;   % 3×3
    
        % 3) ∂n̂/∂r  and ∂n̂/∂v
        %    h = r×v  ⇒  ∂h/∂r_j = skew(e_j)*v,   ∂h/∂v = skew(r)
        Sr = skew(r);
        Dh_dr = zeros(3,3);
        for j=1:3,  ej = zeros(3,1); ej(j)=1;
          Dh_dr(:,j) = skew(ej)*v;
        end
        Dh_dv = Sr;
        %    n̂ = h/h_norm  ⇒  Dn = (I−n̂ n̂')/h_norm 
        Dn = (eye(3)-n_hat*n_hat')/h_norm;
        Dn_dr = Dn*Dh_dr;
        Dn_dv = Dn*Dh_dv;
    
        % 4) ∂t̂/∂r  and ∂t̂/∂v
        %    q = h×r  ⇒  ∂q/∂r = skew(Dh_dr(:,j))*r + skew(h)*I,   ∂q/∂v = skew(Dh_dv(:,j))*r
        Sh = skew(h_vec);
        Dq_dr = zeros(3,3);
        for j=1:3
            Dq_dr(:,j) = skew(Dh_dr(:,j))*r + Sh(:,j);  
        end
        Dq_dv = zeros(3,3);
        for j=1:3
            Dq_dv(:,j) = skew(Dh_dv(:,j))*r;           
        end
        %    t̂ = q/q_norm  ⇒  Dt = (I−t̂ t̂')/q_norm
        Dt = (eye(3)-t_hat*t_hat')/q_norm;
        Dt_dr = Dt*Dq_dr;
        Dt_dv = Dt*Dq_dv;

        Drng_dr = A1*(r_hat*Dg + g_r*Drhat) + A2*Dt_dr + A3*Dn_dr;
        Drng_dv =                 A2*Dt_dv + A3*Dn_dv;
        ...
        A(4:6,1:3) = dadr_earth + dadr_sun + dadr_srp + Drng_dr;

        % d a_total / d C_r 
        A(4:6,7) = a_srp / Cr;

        % d a_total / d non_grav 
        A(4:6,8) = g_r * r_hat;
        A(4:6,9) =  g_r * t_hat;
        A(4:6,10) =  g_r * n_hat;
...
function results = lkf(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    for t = 1:T
        ...
        if nargin > 3
            ...
            Q_disc = zeros(n);
            Q_disc(1:6, 1:6) = calculate_Q_discrete(dt, x_ref(1:6), ...
                settings_PN);
        else 
            Q_disc = zeros(n); % 1e-8 * eye(n);
        end

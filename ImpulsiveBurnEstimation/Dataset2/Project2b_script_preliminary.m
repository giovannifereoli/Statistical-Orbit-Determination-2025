...
grid on;
view(2);
exportgraphics(gca123, 'ortbi3.pdf', 'ContentType','vector');
...
% Plot Residuals 
gca1233 = figure('Position', [100, 100, 1400, 900]);
subplot(2, 1, 1);
hold on;
for s = 1:num_stations
...
for s = 1:num_stations
    ...
    % Store time and residuals
    time_per_station{s} = trackingData.time(station_indices); % Time in seconds
    range_residuals{s} = cellfun(@(x) x(1), {measurements_struct2(station_indices).residual});
    range_rate_residuals{s} = cellfun(@(x) x(2), {measurements_struct2(station_indices).residual});
end
...
% results = lkf(trajectory_ref2, measurements_struct2, P0);
...
gca789 = figure('Position', [100, 100, 1400, 900]);
...
exportgraphics(gca789, 'postfit_lkf_b.pdf', 'ContentType','vector');

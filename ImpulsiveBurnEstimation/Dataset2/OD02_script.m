...
state0_true = [r0; v0; par0; reshape(eye(11), [], 1)];
...
[t_ref, state_ref] = propagate_flyby_with_impulse(state0_true, JD0, mu_E, mu_S, AMR, Pphi, AU, t_span, options);
...
% STM reshape
STM_ref = reshape(state_ref(:, 12:end), [], 11, 11);
...
for i = 1:length(t_ref)
    ...
    trajectory_ref(i).parameters = state_ref(i,7:11);
...
[t_ref2, state_ref2] = propagate_flyby_with_impulse([results.state_corrected_hist(:, 1); eyeI(:)], JD0, ...
    mu_E, mu_S, AMR, Pphi, AU, t_span, options);


% STM reshape
STM_ref2 = reshape(state_ref2(:, 12:end), [], 11, 11);
...
for i = 1:length(t_ref)
    ...
    trajectory_ref2(i).parameters = state_ref2(i,7:11);
...
% 
% % Integrate trajectory
% [t_ref3, state_ref3] = propagate_flyby_with_impulse([results.state_corrected_hist(:, 1); eyeI(:)], JD0, ...
%     mu_E, mu_S, AMR, Pphi, AU, t_span, options);
% 
% % STM reshape
% STM_ref3 = reshape(state_ref2(:, 12:end), [], 11, 11);
% 
% % Preallocate structs
...
%     trajectory_ref3(i).parameters = state_ref3(i,7:11);
...
% Updated state labels (10-state example)
labels = {'$x$', '$y$', '$z$', ...
          '$\dot{x}$', '$\dot{y}\cdot 10^{-5}$', '$\dot{z}\cdot 10^{-5}$', ...
          '$C_R$', ...
          '$\Delta v_x\cdot 10^{-7}$', '$\Delta v_y\cdot 10^{-6}$', '$\Delta v_z\cdot 10^{-5}$', ...
          '$t_b$'};
...
% Plot
gca123 = figure('Position', [100, 100, 1400, 1000]);
for i = 1:n_states
    subplot(4, 3, i);
...
exportgraphics(gca123, 'OD_DV_sigmas.pdf', 'ContentType','vector');
...
% Right: Histogram
nexttile(4);
histogram(rangerate_res, 500, 'Orientation', 'horizontal', ...
...
legend({...
    ...
    'Interpreter', 'latex', 'FontSize', 11, 'Location', 'northeast');
exportgraphics(gca345, 'OD_DV_postfits.pdf', 'ContentType','vector');
...
t1_DCO    = 10 * t0_DCO;
x0_DCO    = [results.state_corrected_hist(:, end); reshape(eye(11), [], 1) ];

% Propagate until reaching 3 * RSOI
[t_rSOI, state_rSOI] = ode113( ...
    @(t, x) flyby_ODE(t, x, JD0, mu_E, mu_S, AMR, Pphi, AU, flag_STM), ...
    [t0_DCO, t1_DCO], ...
    x0_DCO, ...
    opts_event);
...
Phi_DCO_to_bplane = reshape(state_bplane(end, 12:end), 11, 11);
...
%% B-Plane Sensitivity to Mapping Radius

% Constants
R_SOI    = 924000;  % Earth's SOI radius in km
ratios   = linspace(0.1, 20.0, 10);  % Multiples of SOI radius to test

% Storage for B-plane coordinates
BdotT_array = zeros(size(ratios));
BdotR_array = zeros(size(ratios));

for k = 1:length(ratios)
    r_target = ratios(k) * R_SOI;

    % Propagate until reaching r_target 
    opts_event = odeset('Events', @(t, x) reach_r_event(t, x, r_target), ...
                        'RelTol', 1e-12, 'AbsTol', 1e-14);
    [t_SOI, state_SOI] = ode113(@(t, x) flyby_ODE(t, x, JD0, mu_E, mu_S, AMR, Pphi, AU, 1), ...
                                [t0_DCO, t1_DCO], x0_DCO, opts_event);
    
    % Get position and velocity at event
    r_event = state_SOI(end, 1:3)';
    v_event = state_SOI(end, 4:6)';
    v_hat   = v_event / norm(v_event);

    % Construct B-plane frame
    T_hat = cross(v_hat, [0; 0; 1]); T_hat = T_hat / norm(T_hat);
    R_hat = cross(v_hat, T_hat);
    DCM   = [T_hat, R_hat, v_hat]';

    % Compute B-vector
    B_vec = r_event - dot(r_event, v_event) / norm(v_event)^2 * v_event;
    B_proj = DCM * B_vec;

    % Store B·T and B·R
    BdotT_array(k) = B_proj(1);
    BdotR_array(k) = B_proj(2);
end

% Plot 
gca99 = figure;
plot(ratios, BdotT_array, '-o', 'DisplayName', '$B \cdot \hat{T}$', 'LineWidth', 2); 
hold on;
plot(ratios, BdotR_array, '-s', 'DisplayName', '$B \cdot \hat{R}$', 'LineWidth', 2);
xlabel('Mapping Radius / $R_{SOI}$', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('B-plane Coordinates (km)', 'Interpreter', 'latex', 'FontSize', 14);
legend('Interpreter', 'latex', 'FontSize', 12, 'Location', 'best');
grid on;
exportgraphics(gca99, 'Bplane_sensitvity.pdf', 'ContentType','vector');
...
function dstate = flyby_ODE(t, state, JD0, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM)
    ...
    n = 11;  % 3 position + 3 velocity + 1 Cr + 3 dV impulse parameters + 1 t_burn
    if flag_STM
...
function [T_out, Y_out] = propagate_flyby_with_impulse(y0, JD0, mu_E, mu_S, AM_ratio, Pphi, AU, tspan, options)
    % Initialize
    n = 11;
    flag_STM = 1;
    t_burn = y0(11);
    ...
    sol_pre = ode113(@(t, y) flyby_ODE(t, y, JD0, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM), ...
    ...
    sol_post = ode113(@(t, y) flyby_ODE(t, y, JD0, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM), ...
...
function measurements_struct = radiometric_measurement(trackingData, sc_ref, ...
    ...
    for i = 1:N
        ...
        d_measurements_dX = [d_rho_dR, d_rho_dV, 0, 0, 0, 0, 0;
                             d_rho_dot_dR, d_rho_dot_dV, 0, 0, 0, 0, 0];
...
function results = lkf(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    for t = 1:T
        ...
        if nargin > 3
            ...
            Q_6x6 = calculate_Q_discrete(dt, x_ref(1:6), settings_PN); 
            Q_disc = zeros(n); 
            Q_disc(1:6, 1:6) = Q_6x6;
        else 
            Q_disc = eye(n);
        end

...
% ASSIGNMENT:   Project 2 - Part 2b - DV
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear; clc; close all;
...
par0 = [Cr; dV0; t_burn; 0; 0; 0]; 

% Initial full state vector: [x; v; Cr; STM]
state0_true = [r0; v0; par0; reshape(eye(14), [], 1)];
...
[t_ref, state_ref] = propagate_flyby_with_impulse(state0_true, JD0, B, mu_E, mu_S, AMR, Pphi, AU, t_span, options);
...
% STM reshape
STM_ref = reshape(state_ref(:, 15:end), [], 14, 14);
...
for i = 1:length(t_ref)
    ...
    trajectory_ref(i).parameters = state_ref(i,7:14);
...
sigma_dV = 1e-4;       % Std for impulsive burn delta-V components (km/s)
sigma_tb = 1e3;        % Std for burn time (s)
sigma_w = 1e-9;
P0 = diag([sigma_pos^2, sigma_pos^2, sigma_pos^2, ...
           sigma_vel^2, sigma_vel^2, sigma_vel^2, ...
           sigma_Cr^2, ...
           sigma_dV^2, sigma_dV^2, sigma_dV^2, ...
           sigma_tb^2, sigma_w^2, sigma_w^2, sigma_w^2]);
...
settings_PN.B = B;
optimal_sigma_Q_cont = 2 * 1e-11;  % Continuous-time covariance matrix (in km/s^2)
...
[t_ref2, state_ref2] = propagate_flyby_with_impulse([results.state_corrected_hist(:, 1); eyeI(:)], JD0, B, ...
    mu_E, mu_S, AMR, Pphi, AU, t_span, options);


% STM reshape
STM_ref2 = reshape(state_ref2(:, 15:end), [], 14, 14);
...
for i = 1:length(t_ref)
    ...
    trajectory_ref2(i).parameters = state_ref2(i,7:14);
...
measurements_struct2 = radiometric_measurement(trackingData, state_ref2', ...
    station_ecef0, R, results.state_corrected_hist(8:10, 1), ...
    results.state_corrected_hist(11, 1));
...
% 
% % Integrate trajectory
% [t_ref3, state_ref3] = propagate_flyby_with_impulse([results.state_corrected_hist(:, 1); eyeI(:)], JD0, B, ...
%     mu_E, mu_S, AMR, Pphi, AU, t_span, options);
% 
% % STM reshape
% STM_ref3 = reshape(state_ref2(:, 15:end), [], 14, 14);
% 
% % Preallocate structs
...
%     trajectory_ref3(i).parameters = state_ref3(i,7:14);
...
labels = {'$x$', '$y$', '$z$', ...
          '$\dot{x}$', '$\dot{y}$', '$\dot{z}$', ...
          '$C_R$', ...
          '$\Delta v_1\cdot 10^{-6}$', '$\Delta v_2\cdot 10^{-6}$', ...
          '$\Delta v_3\cdot 10^{-5}$', ...
          '$t_{\mathrm{burn}}$', ...
          '$w_x$', '$w_y$', '$w_z$'};
...
y_labels = {'Position Error (km)', ...
            'Velocity Error (km/s)', ...
            'SRP Coeff. Error', ...
            'Empirical $\Delta v_i$ Error (km/s)', ...
            'Turn Parameter Error', ...
            'Empirical Accel. $w_i$ Error (km/s$^2$)'};

% Colors
cmap = lines(n_states);

% Plot
gca123 = figure('Position', [100, 100, 1600, 1000]);
for i = 1:n_states
    subplot(5, 3, i);
    ...
    elseif i == 11
        ylabel(y_labels{5}, 'Interpreter', 'latex', 'FontSize', 14);
    else
        ylabel(y_labels{6}, 'Interpreter', 'latex', 'FontSize', 14);
    end
...
exportgraphics(gca123, 'OD_DMC_DV_sigmas.pdf', 'ContentType','vector');
...
legend({...
    ...
    'Interpreter', 'latex', 'FontSize', 11, 'Location', 'northeast');
exportgraphics(gca345, 'OD_DMC_DV_postfits.pdf', 'ContentType','vector');
...
w_hist = results.state_corrected_hist(12:14, :);  % [wx; wy; wz]
N = size(w_hist, 2);

% Scatter plot of empirical accelerations
gca567 = figure('Position', [100, 100, 1200, 500]);
scatter(t_hours, w_hist(1,:), 20, 'r', 'filled'); hold on;
scatter(t_hours, w_hist(2,:), 20, 'g', 'filled');
scatter(t_hours, w_hist(3,:), 20, 'b', 'filled');
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Empirical Acceleration $\cdot 10^{-8}$ (km/s$^2$)', 'Interpreter', 'latex', 'FontSize', 14);
legend({'$w_x$', '$w_y$', '$w_z$'}, 'Interpreter', 'latex', 'FontSize', 12, 'Location', 'best');
grid on;
exportgraphics(gca567, 'OD_DMC_DV_wvec.pdf', 'ContentType','vector');
...
ylabel('FFT Magnitude', 'Interpreter', 'latex', 'FontSize', 14);
title('Fourier Spectrum of Empirical Acceleration Magnitude', 'Interpreter', 'latex', 'FontSize', 16);

% Plot: Magnitude, In-Plane, and Out-of-Plane components

% In-plane = projection onto orbital plane (assume inertial plane XY)
% Out-of-plane = Z component of unit vector
w_in_plane = sqrt(w_unit(1,:).^2 + w_unit(2,:).^2);
w_out_plane = abs(w_unit(3,:));

figure('Position', [100, 100, 1000, 600]);
subplot(3, 1, 1);
scatter(t_hours, w_mag, 20, 'k', 'filled'); grid on;
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$\| \mathbf{w}(t) \|$ (km/s$^2$)', 'Interpreter', 'latex', 'FontSize', 14);
...
t1_DCO    = 10 * t0_DCO;
x0_DCO    = [results.state_corrected_hist(:, end); reshape(eye(14), [], 1) ];
x0_DCO(8:10) = 0; 
...
Phi_DCO_to_bplane = reshape(state_bplane(end, 15:end), 14, 14);
...
% theta = linspace(0, 2*pi, 200);  % circle parameter
% x_earth = R_earth * cos(theta);  % x-coordinates (BdotT axis)
% y_earth = R_earth * sin(theta);  % y-coordinates (BdotR axis)
% plot(x_earth, y_earth, 'k-', 'LineWidth', 2);  % black solid circle
grid on; box on;
...
n_estimated = 14; % [x,y,z, vx,vy,vz, Cr, wx, wy, wz, dvx, dvy, dvz, tb]
...
%% Final State Estimate and Uncertainties at t0

fprintf('\n========= Final Estimated State at t0 =========\n');
state_labels = { ...
    'x       (km)', ...
    'y       (km)', ...
    'z       (km)', ...
...
x_hat_0 = results.state_corrected_hist(:,1);
P0_final = results.P_hist(:,:,1);
sigmas_0 = sqrt(diag(P0_final));

for i = 1:14
    fprintf('%-10s = %+ .14f  ±  %.14f\n', state_labels{i}, x_hat_0(i), sigmas_0(i));
end
fprintf('==============================================\n');
...
function dstate = flyby_ODE(t, state, JD0, B, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM)
    ...
    n = 14;  % 3 position + 3 velocity + 1 Cr + 3 dV impulse parameters + 1 t_burn
    if flag_STM
    ...
    % dV      = state(8:10);     % Impulse delta-V vector [km/s]
    % t_burn  = state(11);       % Burn time [s]
    w     = state(12:14);
    ...
    dstate(8:10) = zeros(3,1);  % dV constant
    dstate(11)   = 0;           % t_burn constant
    dstate(12:14) = - B * w;
...
function [T_out, Y_out] = propagate_flyby_with_impulse(y0, JD0, B, mu_E, mu_S, AM_ratio, Pphi, AU, tspan, options)
    % Initialize
    n = 14;
    flag_STM = 1;
    t_burn = y0(11);

    % Split the tspan for pre- and post-burn phases
    t_pre_idxs = tspan <= t_burn;
    t_post_idxs = tspan > t_burn;
    t_pre  = tspan(t_pre_idxs);
    t_post = tspan(t_post_idxs);
    if isempty(t_pre) || t_pre(end) < t_burn
        t_pre = [t_pre; t_burn];
    end
    if isempty(t_post) || t_post(1) > t_burn
        t_post = [t_burn; t_post];
    end

    % Phase A: pre-burn propagation (dense output)
    sol_pre = ode113(@(t, y) flyby_ODE(t, y, JD0, B, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM), ...
                     [t_pre(1), t_pre(end)], y0, options);
    Y_pre = deval(sol_pre, t_pre(1:end-1)).';         % Keep full pre-burn times

    % Apply burn
    y_burn = deval(sol_pre, t_burn);         % This ensures you're at exact t_burn
    dV = y0(8:10);
    Phi = reshape(y_burn(n+1:end), n, n);
    Phi_rv = Phi(1:6, 1:6);
    y_burn(4:6) = y_burn(4:6) + dV;
    Phi(1:6, 8:10) = Phi_rv * [zeros(3); eye(3)];
    Phi(1:6, 11)   = Phi_rv * [-dV; zeros(3,1)];
    y_burn(n+1:end) = Phi(:); 
    % y_burn(8:10) = 0;

    % Phase B: post-burn propagation (dense output)
    if isempty(t_post)
        T_out = t_pre;
        Y_out = Y_pre;
        return;
    end
    sol_post = ode113(@(t, y) flyby_ODE(t, y, JD0, B, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM), ...
                      [t_post(1), t_post(end)], y_burn, options);
    Y_post = deval(sol_post, t_post(2:end)).';

    % Stitch results
    T_out = [t_pre(1:end-1); t_post(2:end)];
    Y_out = [Y_pre; Y_post];
end
...
function measurements_struct = radiometric_measurement(trackingData, sc_ref, ...
    ...
    for i = 1:N
        ...
        d_measurements_dX = [d_rho_dR, d_rho_dV, 0, 0, 0, 0, 0, 0, 0, 0;
                             d_rho_dot_dR, d_rho_dot_dV, 0, 0, 0, 0, 0, 0, 0, 0];

        % Store function handle for measurement model
        measurement_func = @(x) measurement_function(trackingData.time(i), x, ...
            gs_pos, gs_vel, t_burn, dV_vec);
...
function [h_x, H] = measurement_function(~, x, gs_pos, gs_vel, ~, ~)
    ...
    H = [d_rho_dR, d_rho_dV, 0, 0, 0, 0, 0, 0, 0, 0;
         d_rho_dot_dR, d_rho_dot_dV, 0, 0, 0, 0, 0, 0, 0, 0];
end
...
function results = lkf(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    for t = 1:T
        ...
        if nargin > 3 % && measurement_times(t) <= x_ref(11)
            ...
            Q_disc(12:14, 12:14) = Q_9x9(7:9, 7:9);
            
            % Fill in cross-terms (if any) between r/v and empirical acceleration
            Q_disc(1:6, 12:14) = Q_9x9(1:6, 7:9);
            Q_disc(12:14, 1:6) = Q_9x9(7:9, 1:6);

        % elseif nargin > 3 && measurement_times(t) > x_ref(11)
        %     dt = 0;  % Default value for the first iteration
        %     if t > 1
        %         dt = measurement_times(t) - measurement_times(t-1); % Time step
        %     end
        %     settings_PN.Q_cont = (10 * 1e-12)^2 * eye(3);
        %     Q_9x9 = calculate_Q_discrete(dt, x_ref(1:6), settings_PN);  % 9x9
        %     Q_disc = zeros(n);  % Final 10x10 output
        % 
        %     % Fill in Q(1:6,1:6) – position and velocity

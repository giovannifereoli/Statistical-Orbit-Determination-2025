...
state0_true = [r0; v0; par0; reshape(eye(10), [], 1)];
...
for i = 1:length(t_ref)
    ...
    trajectory_ref(i).parameters = state_ref(i,7:10);
...
settings_PN.B = B;
optimal_sigma_Q_cont = 1e-11;  % Continuous-time covariance matrix (in km/s^2)
...
results = rts_smoother(results, trajectory_ref, measurements_struct);
...
% STM reshape
STM_ref2 = reshape(state_ref2(:, 11:end), [], 10, 10);
...
for i = 1:length(t_ref)
    ...
    trajectory_ref2(i).parameters = state_ref2(i,7:10);
...
% State labels
labels = {'$x$', '$y$', '$z$', ...
          '$\dot{x}$', '$\dot{y}$', '$\dot{z}\cdot 10^{-4}$', ...
          '$C_R$', '$w_x$', '$w_y \cdot 10^{-9}$', '$w_z\cdot 10^{-9}$'};
...
exportgraphics(gca123, 'OD_DMC_sigmas.pdf', 'ContentType','vector');
...
gca345 = figure('Position', [100, 100, 1400, 800]);
...
legend({...
    ...
    'Interpreter', 'latex', 'FontSize', 11, 'Location', 'northeast');
exportgraphics(gca345, 'OD_DMC_postfits.pdf', 'ContentType','vector');

%% Empirical Acceleration Analysis and Fourier Spectrum - Pt.1

% Time vector in hours
t_hours = [measurements_struct.time] / 3600;
w_hist = results.state_corrected_hist(8:10, :);  % [wx; wy; wz]
N = size(w_hist, 2);

% Scatter plot of empirical accelerations
gca10 = figure('Position', [100, 100, 1200, 500]);
scatter(t_hours, w_hist(1,:), 20, 'r', 'filled'); hold on;
scatter(t_hours, w_hist(2,:), 20, 'g', 'filled');
scatter(t_hours, w_hist(3,:), 20, 'b', 'filled');
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Empirical Acceleration $\cdot 10^{-9}$ (km/s$^2$)', 'Interpreter', 'latex', 'FontSize', 14);
legend({'$w_x$', '$w_y$', '$w_z$'}, 'Interpreter', 'latex', 'FontSize', 12, 'Location', 'best');
grid on;
exportgraphics(gca10, 'OD_DMC_wvec.pdf', 'ContentType','vector');

% Magnitude and normalized direction
w_mag = sqrt(sum(w_hist.^2, 1));         % 1 x N
w_unit = w_hist ./ (w_mag + 1e-20);      % unit vectors

% Fourier Transform on magnitude
dt = mean(diff(t_hours)) * 3600;         % [s]
fs = 1 / dt;
W_fft = fft(w_mag);
f = fs * (0:(N/2)-1) / N;
T = 1 ./ f / 3600;                       % Periods in hours
W_fft_mag = abs(W_fft(1:N/2));

% Plot FFT magnitude vs period
gca11 = figure('Position', [100, 100, 1000, 500]);
plot(T / 24, W_fft_mag, 'k-', 'LineWidth', 2); grid on;
xlim([min(T(T>0)), max(T)]);  % Skip Inf
set(gca, 'XScale', 'log');
xlabel('Period (days)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('FFT Magnitude $\cdot 10^{-6}$', 'Interpreter', 'latex', 'FontSize', 14);
exportgraphics(gca11, 'OD_DMC_fft.pdf', 'ContentType','vector');

% Plot: Magnitude, In-Plane, and Out-of-Plane components

% In-plane = projection onto orbital plane (assume inertial plane XY)
gca12 = figure('Position', [100, 100, 1000, 600]);
subplot(3, 1, 1);
scatter(t_hours, w_mag, 20, 'k', 'filled'); grid on;
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$\| \mathbf{w}(t) \| \cdot 10^{-9}$ (km/s$^2$)', 'Interpreter', 'latex', 'FontSize', 14);
title('Empirical Acceleration Magnitude', 'Interpreter', 'latex', 'FontSize', 16);

% Compute spherical angles from unit vectors
wx = w_unit(1,:);
wy = w_unit(2,:);
wz = w_unit(3,:);

% Azimuth phi (in-plane): atan2(wy, wx), [-pi, pi]
phi_rad = atan2(wy, wx);
phi_deg = rad2deg(phi_rad);  % for plotting
phi_deg = mod(phi_deg, 360); % Wrap into [0, 360)

% Elevation theta: asin(wz), [-pi/2, pi/2]
theta_rad = asin(wz);
theta_deg = rad2deg(theta_rad);

% Plot angles over time
subplot(3,1,2);
scatter(t_hours, phi_deg, 20, 'filled', 'MarkerFaceColor', [0.2 0.4 0.8]); grid on;
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$\phi$ (deg)', 'Interpreter', 'latex', 'FontSize', 14);
title('Azimuth Angle of Empirical Acceleration ($\phi$)', 'Interpreter', 'latex', 'FontSize', 16);

subplot(3,1,3);
scatter(t_hours, theta_deg, 20, 'filled', 'MarkerFaceColor', [0.8 0.2 0.5]); grid on;
xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$\theta$ (deg)', 'Interpreter', 'latex', 'FontSize', 14);
title('Elevation Angle of Empirical Acceleration ($\theta$)', 'Interpreter', 'latex', 'FontSize', 16);
exportgraphics(gca12, 'OD_DMC_wanalysis1.pdf', 'ContentType','vector');

% Plot: Average Magnitude vs Moving Window Length (0 to 90 days)

% Define moving window lengths from 0.5 hours up to 90 days
window_lengths_days = linspace(0.01, 90, 300);  % 300 points from 0.01 to 90 days
window_lengths_sec = window_lengths_days * 24 * 3600;  % convert days to seconds
avg_magnitudes = zeros(size(window_lengths_days));

for i = 1:length(window_lengths_days)
    window_size = window_lengths_sec(i);   % window size in seconds
    window_samples = max(1, round(window_size / dt));  % number of samples per window
    % Moving average of w_mag
    w_mag_movavg = movmean(w_mag, window_samples);
    % Average magnitude over the whole arc
    avg_magnitudes(i) = mean(w_mag_movavg);
end

% Plot
gca13 = figure('Position', [100, 100, 1000, 500]);
plot(window_lengths_days, avg_magnitudes * 1e9, 'o-k', 'LineWidth', 2, 'MarkerSize', 4, 'MarkerFaceColor', 'k'); grid on;
xlabel('Moving Window Length (days)', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Average Magnitude $\cdot 10^{-9}$ (km/s$^2$)', 'Interpreter', 'latex', 'FontSize', 14);
xlim([0 90]);
exportgraphics(gca13, 'OD_DMC_movavg_vs_window_days.pdf', 'ContentType','vector');


%% Final Solutions
% 
% % Time vector from tracking data
% t_span = trackingData.time;
% N_times = length(t_span);
% 
% % ODE settings
% options = odeset('RelTol', 2.22045e-12, 'AbsTol', 2.22045e-14);
% flag_STM = 1;
% eyeI = eye(10);  % Identity matrix for STM
% 
% % Initialize outputs
% t_all = [];
% state_all = [];
% 
% fprintf('Propagating trajectory segment-by-segment:   0%%');
% 
% % Segment-by-segment propagation
% for k = 1:N_times-1
%     % Initial state = corrected filter solution at time k
%     xk = results.state_corrected_hist(:, k);
%     xk_aug = [xk; eyeI(:)];
% 
%     % Propagate from t_k to t_{k+1}
%     [t_seg, state_seg] = ode113(@(t, state) ...
%         flyby_ODE(t, state, JD0, B, mu_E, mu_S, AMR, Pphi, AU, flag_STM), ...
%         [t_span(k), t_span(k+1)], xk_aug, options);
% 
%     % Append (exclude duplicate at segment boundaries)
%     if k == 1
%         t_all = [t_all; t_seg];
%         state_all = [state_all; state_seg];
%     else
%         t_all = [t_all; t_seg(2:end)];
%         state_all = [state_all; state_seg(2:end,:)];
%     end
% 
%     % Print progress
%     fprintf('\b\b\b\b%3d%%', round(100 * k / (N_times - 1)));
% end
...
%% Alignment of Empirical Acceleration with Sun and Earth Directions

% 1. SC position in ECI [km] from filtered trajectory
r_sc = results.state_corrected_hist(1:3, :);        % [3 x N]
w_hist = results.state_corrected_hist(8:10, :);     % empirical acceleration [3 x N]

% 2. Julian Dates for each epoch
JD_vec = JD0 + t_hours / 24;

% 3. Sun position in ECI (Sun wrt Earth)
N = size(r_sc, 2);
r_sun = zeros(3, N);  % Preallocate

for k = 1:N
    [r_earth_to_sun, ~] = Ephemeride(JD_vec(k), 3, mu_S);  % Earth wrt Sun
    r_sun(:,k) = -r_earth_to_sun(:);  % Sun wrt Earth = - Earth wrt Sun
end

% Preallocate
cos_angle_sun = zeros(1, N);
cos_angle_earth = zeros(1, N);

for k = 1:N
    w_k = w_hist(:,k);
    if norm(w_k) < 1e-20
        continue;  % Skip near-zero acceleration
    end

    w_dir = w_k / norm(w_k);
    sun_dir = r_sun(:,k) / norm(r_sun(:,k));
    earth_dir = -r_sc(:,k) / norm(r_sc(:,k));  % Earth toward SC

    cos_angle_sun(k) = dot(w_dir, sun_dir);
    cos_angle_earth(k) = dot(w_dir, earth_dir);
end

% Filter out NaNs (if any)
valid_sun = ~isnan(cos_angle_sun);
valid_earth = ~isnan(cos_angle_earth);

% Create histograms
gca14 = figure('Position', [100, 100, 1000, 450]);

% Histogram: Alignment with Sun
subplot(1,2,1);
histogram(cos_angle_sun(valid_sun), 50, ...
    'FaceColor', [0.9 0.2 0.2], 'EdgeColor', 'none', 'FaceAlpha', 0.85);
xlabel('$\cos(\alpha_\mathrm{sun})$', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Count', 'Interpreter', 'latex', 'FontSize', 14);
title('Alignment w/ Sun Direction', 'Interpreter', 'latex', 'FontSize', 16);
grid on;
xlim([-1.05, 1.05]);

% Histogram: Alignment with Earth
subplot(1,2,2);
histogram(cos_angle_earth(valid_earth), 50, ...
    'FaceColor', [0.2 0.2 0.9], 'EdgeColor', 'none', 'FaceAlpha', 0.85);
xlabel('$\cos(\alpha_\mathrm{earth})$', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Count', 'Interpreter', 'latex', 'FontSize', 14);
title('Alignment w/ Earth Direction', 'Interpreter', 'latex', 'FontSize', 16);
grid on;
xlim([-1.05, 1.05]);
exportgraphics(gca14, 'OD_DMC_wanalysis.pdf', 'ContentType','vector');

%% B-plane Target Computation
...
t1_DCO    = 10 * t0_DCO;
x0_DCO    = [results.state_corrected_hist(:, end); reshape(eye(10), [], 1) ];
x0_DCO(8:10) = 0; 

% Propagate until reaching 3 * RSOI
[t_rSOI, state_rSOI] = ode113( ...
    @(t, x) flyby_ODE(t, x, JD0, zeros(3), mu_E, mu_S, AMR, Pphi, AU, flag_STM), ...
    [t0_DCO, t1_DCO], ...
    x0_DCO, ...
    opts_event);
...
Phi_DCO_to_bplane = reshape(state_bplane(end, 11:end), 10, 10);
...
fname = sprintf('bplane_%s.mat', solution_name);
save(fname, 'Rstruct');
...
% 8) Prints
fprintf( '\n======= B-PLANE INFO =======\n');
fprintf( 't_DCO (days): %.6f\n', t_rSOI(1)/86400 );
fprintf( 'r - 3 * R_SOI (should be small): %.14f km\n', norm(state_rSOI(end,1:3)) - r_target );
fprintf( 't_3rsoi (days): %.6f\n', t_rSOI(end)/86400 );
fprintf( 'BdotT = %.3f km\n', BdotT);
fprintf( 'BdotR = %.3f km\n', BdotR);
fprintf( 't_bplane (days): %.6f\n', t_bplane(end)/86400 );
fprintf( 'BdotS (should be small): %.14f km\n', dot(r_bplane, S_hat));

% Compute 1-sigma uncertainties along T and R
sigma_BdotT = sqrt(P_r_B(1,1));
sigma_BdotR = sqrt(P_r_B(2,2));

fprintf('1-sigma BdotT uncertainty: %.6f km\n', sigma_BdotT);
fprintf('1-sigma BdotR uncertainty: %.6f km\n', sigma_BdotR);
fprintf('=============================\n');
...
n_estimated = 10; % [x,y,z, vx,vy,vz, Cr, wx, wy, wz]
...
%% Fitting Empirical Accelerations
% 
% % Assume: 
% % t_hours = time vector in hours (Nx1)
% % w_hist = empirical acceleration [3 x N] (from filter)
% 
% degree = 4;  % Degree of polynomial fit
% 
% % Fit w_x(t)
% p_x = polyfit(t_hours, w_hist(1,:), degree);
% 
% % Fit w_y(t)
% p_y = polyfit(t_hours, w_hist(2,:), degree);
% 
% % Fit w_z(t)
% p_z = polyfit(t_hours, w_hist(3,:), degree);
% 
% w_x = polyval(p_x, t_hours);
% w_y = polyval(p_y, t_hours);
% w_z = polyval(p_z, t_hours);
% 
% % Plot
% figure('Position', [100, 100, 1400, 900]);
% component_labels = {'$w_x$', '$w_y$', '$w_z$'};
% colors = [1 0 0; 0 0.6 0; 0 0 1];  % red, green, blue
% 
% for i = 1:3
%     subplot(3,1,i); hold on;
% 
%     % Plot true
%     scatter(t_hours, w_hist(i,:), 20, 'MarkerFaceColor', colors(i,:), ...
%             'MarkerEdgeColor', colors(i,:), 'DisplayName', 'Estimated Points');
% 
%     % Plot fit
%     if i == 1
%         plot(t_hours, w_x, '-', 'Color', colors(i,:), 'LineWidth', 2, ...
%             'DisplayName', 'Polynomial Fit');
%     elseif i == 2
%         plot(t_hours, w_y, '-', 'Color', colors(i,:), 'LineWidth', 2, ...
%             'DisplayName', 'Polynomial Fit');
%     else
%         plot(t_hours, w_z, '-', 'Color', colors(i,:), 'LineWidth', 2, ...
%             'DisplayName', 'Polynomial Fit');
%     end
% 
%     % Labels
%     xlabel('Time (hours)', 'Interpreter', 'latex', 'FontSize', 14);
%     ylabel(sprintf('%s (km/s$^2$)', component_labels{i}), 'Interpreter', 'latex', 'FontSize', 14);
%     title(sprintf('Empirical Acceleration: %s', component_labels{i}), 'Interpreter', 'latex', 'FontSize', 16);
...
function dstate = flyby_ODE(t, state, JD0, B, mu_E, mu_S, AM_ratio, Pphi, AU, flag_STM)
    % Determine state size
    n = 10;  % 3 position + 3 velocity + 1 parameter (Cr)
    if flag_STM
...
function measurements_struct = radiometric_measurement(trackingData, sc_ref, ...
    ...
    for i = 1:N
        ...
        d_measurements_dX = [d_rho_dR, d_rho_dV, 0, 0, 0, 0;
                             d_rho_dot_dR, d_rho_dot_dV, 0, 0, 0, 0];
...
function results = lkf(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    for t = 1:T
        ...
        if nargin > 3
            ...
            % Fill in Q(8:10,8:10) – empirical accelerations
            Q_disc(8:10, 8:10) = Q_9x9(7:9, 7:9);
            
            % Fill in cross-terms (if any) between r/v and empirical acceleration
            Q_disc(1:6, 8:10) = Q_9x9(1:6, 7:9);
            Q_disc(8:10, 1:6) = Q_9x9(7:9, 1:6);
        else 
            Q_disc = zeros(n);
        end

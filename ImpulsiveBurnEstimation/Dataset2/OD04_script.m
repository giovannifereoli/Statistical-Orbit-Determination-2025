...
results = srif(trajectory_ref, measurements_struct, P0, settings_PN);
...
results = srif(trajectory_ref2, measurements_struct2, P0, settings_PN);
...
% Right: Histogram
nexttile(4);
histogram(rangerate_res, 1000, 'Orientation', 'horizontal', ...
...
function [Q_disc, Gamma] = calculate_Q_discrete(dt, state, settings)
...
function results = srif(trajectory_ref, sorted_measurements, P0, settings_PN)
    ...
    for t = 1:T
        ...
        prefit_residual = sorted_measurements(t).residual;
        H_tilde = sorted_measurements(t).partials.wrt_X;
        R_meas = sorted_measurements(t).covariance;
        
        % Prediction Step: Propagate information matrix
        % OSS: info_vector prediction is useless w/o Q!
        R_sqrt = R_sqrt / STM_dt;

        % Apply Process Noise, if present
        if nargin > 3 && ~isempty(settings_PN)
            ...
            [~, gamma_k_red] = calculate_Q_discrete(dt, x_ref, settings_PN);
            gamma_k = zeros(n, 3); 
            gamma_k(1:6, 1:3) = gamma_k_red;

            % Process noise dimension and Q_cont
            q = 3;  % Adjust if needed or derive from settings_PN
            Q_cont = settings_PN.Q_cont;
            
            % Compute the square root information matrix of Q_cont (Ru_sqrt is q x q)
            Ru_sqrt = chol(inv(Q_cont), 'upper');
            
            % Construct the propagation cost function:
            % OSS: bu is typically zero, because noise has zero mean!
            prediction_matrix = [Ru_sqrt, zeros(q, n), zeros(q, 1); ...
                -R_sqrt * gamma_k, R_sqrt, info_vector];  % size: (q+n) x (q+n+1)
            
            % Apply QR decomposition to the propagation matrix
            [~, propagated_factor] = qr(prediction_matrix);
            
            % Extract the updated information square root and vector from the QR result
            R_sqrt = propagated_factor(q+1:q+n, q+1:q+n);
            info_vector = propagated_factor(q+1:q+n, q+n+1);
        else
            % Construct the reduced prediction matrix
            prediction_matrix = [R_sqrt, info_vector];
            
            % Apply QR decomposition
            [~, propagated_factor] = qr(prediction_matrix);
            
            % Extract the updated information square root and vector
            R_sqrt = propagated_factor(1:n, 1:n);
            info_vector = propagated_factor(1:n, n+1);
        end
